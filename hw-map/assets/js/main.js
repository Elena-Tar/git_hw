...
function go() {
    ...
    for (let i = 0; i < PLACES.length; i++) {
        let marker = L.marker([PLACES[i].lat, PLACES[i].lng], { icon: new LeafIcon({ iconUrl: 'assets/img/leaf-' + PLACES[i].type + '.png' }) });
        marker.bindPopup(`<ul class = "comment">
             <li><img src="${PLACES[i].pic}" alt="#"> </li> 
             <li class="pic_img">${PLACES[i].title} </li>
            </ul>
            `);
